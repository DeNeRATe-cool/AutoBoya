...
PyPI 包名：[`autoboya`](https://pypi.org/project/autoboya/)（当前版本 `0.1.5`）。
...
如果同一用户自动选择同一门课程时连续 3 次收到博雅业务层失败结果，后台会把这门课记录为该用户的失败课程，后续不再自动重试。网络超时、SSL、DNS 等临时连接问题不会计入这 3 次。失败记录存放在 `~/.autoboya/cache/action_journal.json`。

验证码处理方式：命令会下载 SSO 验证码图片并提示操作者手动输入。
